...
// state.
func (obj *ocsCephBlockPools) ensureCreated(r *ReconcileStorageCluster, instance *ocsv1.StorageCluster) error {
	...
	for _, cephBlockPool := range cephBlockPools {
		...
		case err == nil:
			...
			r.reqLogger.Info(fmt.Sprintf("Restoring original cephBlockPool %s", cephBlockPool.Name))
		...
		case errors.IsNotFound(err):
			r.reqLogger.Info(fmt.Sprintf("Creating cephBlockPool %s", cephBlockPool.Name))
...
func (obj *ocsCephBlockPools) ensureDeleted(r *ReconcileStorageCluster, sc *ocsv1.StorageCluster) error {
	...
	for _, cephBlockPool := range cephBlockPools {
		...
		if err != nil {
			if errors.IsNotFound(err) {
				r.reqLogger.Info("Uninstall: CephBlockPool not found", "CephBlockPool Name", cephBlockPool.Name)
				continue
			}
		...
		if cephBlockPool.GetDeletionTimestamp().IsZero() {
			r.reqLogger.Info("Uninstall: Deleting cephBlockPool", "CephBlockPool Name", cephBlockPool.Name)
		...
		if err != nil {
			if errors.IsNotFound(err) {
				r.reqLogger.Info("Uninstall: CephBlockPool is deleted", "CephBlockPool Name", cephBlockPool.Name)
				continue
			}
		}
