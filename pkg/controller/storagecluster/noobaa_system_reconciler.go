...
func (obj *ocsNoobaaSystem) ensureCreated(r *ReconcileStorageCluster, sc *ocsv1.StorageCluster) error {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			r.reqLogger.Info("Waiting on ceph cluster to be created before starting noobaa")
			return nil
		}
		return err
	}
	if !sc.Spec.ExternalStorage.Enable {
		if foundCeph.Status.State != cephv1.ClusterStateCreated {
			r.reqLogger.Info("Waiting on ceph cluster to initialize before starting noobaa")
			return nil
		}
	} else {
		if foundCeph.Status.State != cephv1.ClusterStateConnected {
			r.reqLogger.Info("Waiting for the external ceph cluster to be connected before starting noobaa")
			return nil
		}
	}
	...
	if err != nil {
		r.reqLogger.Error(err, "Failed to create or update NooBaa system")
		return err
	}
...
func (obj *ocsNoobaaSystem) ensureDeleted(r *ReconcileStorageCluster, sc *ocsv1.StorageCluster) error {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			r.reqLogger.Info("Uninstall: NooBaa and noobaa-db PVC not found.")
			return nil
		}
	...
	if !isOwned {
		...
		r.reqLogger.Info("Uninstall: NooBaa object found, but ownerReference not set to storagecluster. Skipping")
		return nil
	}

	if noobaa.GetDeletionTimestamp().IsZero() {
		r.reqLogger.Info("Uninstall: Deleting NooBaa system")
		err = r.client.Delete(context.TODO(), noobaa)
		if err != nil {
			r.reqLogger.Error(err, "Uninstall: Failed to delete NooBaa system")
